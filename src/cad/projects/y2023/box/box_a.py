...
class BoxA:
    ...
    def divider(self, x_dim, y_dim, axis):
        ...
        shape -= box(
            0,
            y_dim - self.wood_thickness - self.tab_width,
            self.wood_thickness,
            y_dim - self.wood_thickness,
        )
        ...
        shape -= self.joint_cut(
            ...
            x_dim,
            offset=True,
            skip_positions=[divider_pos_fn(i) for i in range(n_dividers)] + [0, x_dim],
            tab_width=self.bottom_divider_tab_width,
            start_offset=self.tab_width * 0.5,
        )
    ...
    def get_model(self):
        ...
        m.add_model(self.lid()).renderer.translate(
            z=self.depth - self.wood_thickness + (self.depth * 1.5 if lifted_lid else 0)
        )
