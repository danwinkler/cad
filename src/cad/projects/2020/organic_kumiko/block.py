...
for i in range(4):
    ...
    grid_cut.put(center, out)
    grid_cut.put(out, right)
    grid_cut.put(right, center)
    grid_cut.put(out, left)
    grid_cut.put(center, mid_a)
    grid_cut.put(out, mid_a)
    grid_cut.put(right, mid_a)

    grid_cut.put(center, mid_b)
    grid_cut.put(out, mid_b)
    grid_cut.put(left, mid_b)

outer = max(
    [c for c in grid.get_chordless_cycles() if len(c) > 3], key=lambda c: len(c)
)

cycles = sorted([c for c in grid_cut.get_chordless_cycles()], key=lambda c: len(c))[:-1]

parts = []

holes = []
block = linear_extrude(height=WIDTH + THICKNESS)(make_poly(outer, offset=THICKNESS / 2))

block = intersection()(
    [
        block.copy(),
        translate([-THICKNESS / 2, 0, WIDTH])(rotate(a=90, v=[0, 1, 0])(block.copy())),
        translate([0, WIDTH + THICKNESS / 2, 0])(
            rotate(a=90, v=[1, 0, 0])(block.copy())
        ),
    ]
)

holes += [
    linear_extrude(height=WIDTH + THICKNESS + 2)(make_poly(cycle)) for cycle in cycles
]

z_cut = translate([0, 0, -1])(holes)
x_cut = translate([-1, 0, WIDTH])(rotate(a=90, v=[0, 1, 0])(holes))
y_cut = translate([0, WIDTH + 2, 0])(rotate(a=90, v=[1, 0, 0])(holes))

parts.append(block - z_cut - x_cut - y_cut)
